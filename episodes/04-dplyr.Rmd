...
- "Use `glimpse()` to quickly look at your dataframe."
...
tools for the most common data manipulation tasks. This package is also included larger collection of the `tidyverse` package. It is built to work directly
...
### Taking a quick look at dataframes

Similar to `str()`, which comes built into R, `glimpse()` is a `dplyr` function that (as the name suggests) gives a glimpse of the dataframe.
...
To select columns of a data frame, use `select()`. The first argument to this function is the dataframe (`variants`), and the subsequent arguments are the columns to keep.
