...
For a boxplot, we need one categorical and one continuous 
variable. For our continuous variable, we'll use `RIN` and for our
categorical variable, we'll use `prep_date`. Let's see if our
RNA quality varied a lot between days we ran our protocol. 
...
We can add some additional layers to include a plot title and change the axis labels. Explore the code below and all the different layers that we have added to understand what each layer contributes to the final graphic.

```{r, fig.align='center'}
ggplot(submission_metadata) +
  geom_boxplot(aes(x = prep_date, y = RIN)) +
  ggtitle('RNA quality by preparation date') +
  xlab('preparation date') +
  ylab('RIN') +
